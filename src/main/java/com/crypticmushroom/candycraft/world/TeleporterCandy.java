...
public class TeleporterCandy extends Teleporter {
    ...
    public boolean placeInExistingPortal(Entity entity, float yaw) {
        ...
        if (destinationCoordinateCache.containsKey(1)) {
            Teleporter.PortalPosition portalposition = destinationCoordinateCache.get(1);
            d0 = 0.0D;
            ...
            for (int l = -128; l <= 128; ++l) {
                ...
                for (int eY = -128; eY <= 128; ++eY) {
                    for (BlockPos blockpos1 = blockpos3.add(l, this.world.getActualHeight() - 1 - blockpos3.getY(), eY); blockpos1.getY() >= 0; blockpos1 = blockpos2) {
                        blockpos2 = blockpos1.down();

                        if (this.world.getBlockState(blockpos1).getBlock() == CCBlocks.candyPortal) {
                            while (this.world.getBlockState(blockpos2 = blockpos1.down()).getBlock() == CCBlocks.candyPortal) {
                                blockpos1 = blockpos2;
                            }
        ...
        if (d0 >= 0.0D) {
            ...
            if (patternHelper.getForwards().getOpposite() == enumfacing) {
                f = 1.0F;
                f1 = 1.0F;
            } else if (patternHelper.getForwards().getOpposite() == enumfacing.getOpposite()) {
                f = -1.0F;
                f1 = -1.0F;
            } else if (patternHelper.getForwards().getOpposite() == enumfacing.rotateY()) {
                f2 = 1.0F;
                f3 = -1.0F;
            } else {
                f2 = -1.0F;
            ...
            entity.rotationYaw = yaw - (float)(enumfacing.getOpposite().getHorizontalIndex() * 90) + (float)(patternHelper.getForwards().getHorizontalIndex() * 90);
        ...
        for (int ePosX = entityX - i; ePosX <= entityX + i; ++ePosX) {
            ...
            for (int ePosZ = entityZ - i; ePosZ <= entityZ + i; ++ePosZ) {
                ...
                for (int ePosY = this.world.getActualHeight() - 1; ePosY >= 0; --ePosY) {
                    if (this.world.isAirBlock(mutableBlockPos.setPos(ePosX, ePosY, ePosZ))) {
                        while (ePosY > 0 && this.world.isAirBlock(mutableBlockPos.setPos(ePosX, ePosY - 1, ePosZ))) {
                            --ePosY;
                        }
                            ...
                            for (int j4 = 0; j4 < 3; ++j4) {
                                for (int k4 = 0; k4 < 4; ++k4) {
                                    for (int l4 = -1; l4 < 4; ++l4) {
                                        ...
                                        mutableBlockPos.setPos(mbX, mbY, mbZ);
            ...
            for (int iX = entityX - i; iX <= entityX + i; ++iX) {
                ...
                for (int iZ = entityZ - i; iZ <= entityZ + i; ++iZ) {
                    ...
                    for (int iY = this.world.getActualHeight() - 1; iY >= 0; --iY) {
                        if (this.world.isAirBlock(mutableBlockPos.setPos(iX, iY, iZ))) {
                            while (iY > 0 && this.world.isAirBlock(mutableBlockPos.setPos(iX, iY - 1, iZ))) {
                                --iY;
                            }
                            ...
                            for (int k7 = i2; k7 < i2 + 2; ++k7) {
                                ...
                                for (int j10 = 0; j10 < 4; ++j10) {
                                    for (int j11 = -1; j11 < 4; ++j11) {
                                        ...
                                        int jY = iY + j11;
                                        int jZ = iZ + (j10 - 1) * j9;
                                        mutableBlockPos.setPos(jX, jY, jZ);
            ...
            for (int j7 = -1; j7 <= 1; ++j7) {
                for (int l7 = 1; l7 < 3; ++l7) {
                    for (int k8 = -1; k8 < 3; ++k8) {
                        ...
                        boolean flag = k8 < 0;
                        this.world.setBlockState(new BlockPos(frameX, frameY, frameZ), flag ? CCBlocks.sugarBlock.getDefaultState() : Blocks.AIR.getDefaultState());
                    }
                }
            }
        }

        IBlockState iblockstate = CCBlocks.candyPortal.getDefaultState().withProperty(BlockPortal.AXIS, l6 == 0 ? EnumFacing.Axis.Z : EnumFacing.Axis.X);
        ...
        for (int i8 = 0; i8 < 4; ++i8) {
            for (int l8 = 0; l8 < 4; ++l8) {
                for (int l9 = -1; l9 < 4; ++l9) {
                    ...
                    this.world.setBlockState(new BlockPos(portalX, portalY, portalZ), flag1 ? CCBlocks.sugarBlock.getDefaultState() : iblockstate, 2);
                }
            }
